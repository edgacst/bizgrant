  ...
  return (
    <header className="sticky top-0 inset-x-0 z-50 glass border-b border-gray-200/50 dark:border-gray-700/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <nav className="hidden md:flex items-center justify-center gap-5 flex-1 min-w-0 px-2">
          <Link to="/guide" className={navClass(isGuide)}>
            사용방법
          </Link>
          <Link to="/board" className={`inline-flex items-center gap-1.5 ${navClass(isBoard)}`}>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <Link
            to="/board"
            className={`md:hidden inline-flex items-center gap-1.5 px-2 py-1.5 rounded-lg ${navClass(isBoard)}`}
          >
            <MessageSquare className="w-4 h-4 shrink-0" />
            <span className="text-xs sm:text-sm">게시판</span>
          </Link>
          <DarkModeToggle />
